		 ...
		 Ch04Member member = (Ch04Member) target;
	      ...
	      if(member.getMid()==null | member.getMid().trim().equals("")) {
	         ...
	         if(member.getMid().length()<4) {
	            errors.rejectValue("mid","errors.mid.minlength", "mid는 4자 이상입니다.");
	         }
	      }
	      
	      //mpassword 검사
	      if(member.getMpassword()==null | member.getMpassword().trim().equals("")) {
	         errors.rejectValue("mpassword", "errors.mid.minlength", "mpassword는 필수 입력 사항입니다.");
	      } else {
	         if(member.getMpassword().length()<8) {
	            errors.rejectValue("mpassword", "errors.mpassword.minlength", "mpassword는 8자 이상입니다.");
	         }
	      }
