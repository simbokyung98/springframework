	...
	<div class="card-body">
		<div class="card">
			...
			<script>
				function fileupload() {
					...
					const desc = $("#desc").val();
					const attach = document.querySelector("#attach").files[0];//js 쓰는 이유 -> 파일이란 정보를 얻을 떄 element 안에 있음[제이쿼리 요소 x] 
					...
					$.ajax({
						...
						processData: false, // 쿼리 스트링 형식(title=xxx && dexs=yyy&)이런 형식으로 만들지 말라 
						contentType: false	//파트 마다 Content-Type 이 포함되기때문에 따로 헤더에 Content-Type 추가하지 마라
					}).done((data) => {
						console.log(data);
		...
		<div class="card">
			...
			<div class="card-body">
				...
				<hr/>
				<img width="200px" id="downloadImg"/>
			</div>
